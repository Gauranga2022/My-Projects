...
from sklearn.preprocessing import StandardScaler # Added for potential scaling
import torch
...
if not os.path.exists(data_path):
    ...
    # This might need adjustment based on your execution context
...
# Keep tokenized words as string for now, needed if we use BOW/TFIDF later
...
X = dat.drop(columns=["popular", "tokenized_words"]) # Drop text for now
y = dat["popular"]

X_train_pd, X_test_pd, y_train_pd, y_test_pd = train_test_split(
    X, y, test_size=0.15, random_state=SEED, stratify=y # Added stratify
)
...
def train_model(X_train_np, y_train_np, X_test_np, y_test_np, subset_name, subset_column_list, input_dim, hidden_units, config):
    run_name = f"NN_Full_{subset_name}"
    wandb.init(project=WANDB_PROJECT, entity=ENTITY, name=run_name, config=config, reinit=True)
    print(f"\n--- Training Neural Net - Subset {subset_name} ---")

    # Define model save directory
    model_save_dir = os.path.join("models", "full", subset_name)
    os.makedirs(model_save_dir, exist_ok=True)
    best_model_path = os.path.join(model_save_dir, f'best_model_{subset_name}.pt')

    # Scale numerical features (important for NNs)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train_np)
    X_test_scaled = scaler.transform(X_test_np) # Use transform only on test
    ...
    X_train_tensor, X_val_tensor, y_train_tensor, y_val_tensor = train_test_split(
        X_train_tensor, y_train_tensor, test_size=config["validation_split"], random_state=SEED, stratify=y_train_tensor.cpu().numpy()
    )
    ...
    num_pos = np.sum(y_train_np) # Sum of 1s
    num_neg = len(y_train_np) - num_pos # Total - positive
    pos_weight_val = num_neg / num_pos if num_pos > 0 else 1.0 # Avoid division by zero
    pos_weight_tensor = torch.tensor([pos_weight_val], device=device)
    print(f"Calculated pos_weight for loss: {pos_weight_tensor.item():.4f}")
    ...
    for epoch in range(config["epochs"]):
        ...
        with torch.no_grad():
            for batch_X_val, batch_y_val in val_pbar:
                ...
                logits_val = model(batch_X_val)
                loss_val = criterion(logits_val, batch_y_val)
    ...
    with torch.no_grad():
        for batch_X_test, batch_y_test in test_pbar:
            ...
            predicted_test = (torch.sigmoid(logits_test) > 0.5).float()
    ...
    plt.title(f'Model Accuracy - Subset {subset_name}')
    plt.ylabel('Accuracy')
    plt.xlabel('Epoch')
    plt.legend()
    plt.tight_layout()
    acc_plot_file = f"net_pytorch_acc_{subset_name}.png"
    plt.savefig(acc_plot_file)
    # wandb.log({"accuracy_plot": wandb.Image(acc_plot_file)})
    plt.close()
    ...
    plt.title(f'Model Loss - Subset {subset_name}')
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.legend()
    plt.tight_layout()
    loss_plot_file = f"net_pytorch_loss_{subset_name}.png"
...
config_a = {
    ...
    "epochs": 500,
    "batch_size": 5000,
    "learning_rate": 1e-3, # Adam default, can tune
    "validation_split": 0.13, # ~15% of the 85% train -> ~11% overall
    "early_stopping_patience": 20,
    "hidden_units": 4, # Based on original script for A: Dense(5)->Dense(4)
    "input_dim": len(subset_a_cols),
    "seed": SEED
}
train_model(
    X_train_pd[subset_a_cols].values, y_train_pd.values,
    X_test_pd[subset_a_cols].values, y_test_pd.values,
...
config_b = {
    ...
    "early_stopping_patience": 20,
    "hidden_units": 9, # Based on original script for B: Dense(13)->Dense(9)
    "input_dim": len(subset_b_cols),
    "seed": SEED
}
train_model(
    X_train_pd[subset_b_cols].values, y_train_pd.values,
    X_test_pd[subset_b_cols].values, y_test_pd.values,
